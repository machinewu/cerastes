...
from cerastes.client import YarnApplicationMasterClient, YarnContainerManagerClient
...
def main():
    ...
    # Make container requests requests to ResourceManager
    container_requests = []
    ...
    while completed_containers < n:
      allocate_response = am_client.allocate(asks=container_requests, response_id=response_id)
      ...
      for container in allocate_response.allocated_containers:
        ...
        nm_client = YarnContainerManagerClient(host=container.nodeId.host, port=container.nodeId.port, use_sasl=True)
        ...
        nm_client.start_containers( start_container_request=proto_utils.create_start_container_request( container_launch_context=container_launch_context,
                                                                                                        container_token=associated_token ) )
    ...
    am_client.finish_application_master( final_application_status=proto_utils.FINAL_APPLICATION_STATUS.APP_SUCCEEDED,
                                         diagnostics="",
                                         tracking_url="")
